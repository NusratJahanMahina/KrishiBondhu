...
def generate_id():
    """Generate a unique ID using timestamp plus random number"""
...
def register_login_routes(app):
    ...
    def login():
        ...
        if not login_input or not password:
            flash(get_flash_message('দয়া করে ইউজারনেম/ফোন এবং পাসওয়ার্ড দিন।', 'Please enter username/phone and password.'), 'danger')
        ...
        if user:
            ...
            flash(get_flash_message('ভুল ইউজারনেম/ফোন বা পাসওয়ার্ড।', 'Invalid username/phone or password.'), 'danger')
            return redirect(url_for('login_register'))
    ...
    def register_step3():
        ...
        if request.method == 'POST':
            ...
            try:
                ...
                if count > 0:
                    flash(get_flash_message('এই ফোন নম্বরটি ইতিমধ্যে ব্যবহার করা হয়েছে। দয়া করে ভিন্ন নম্বর দিন।', 'This phone number is already registered. Please use a different one.'), 'danger')
                    session.pop('temp_phone', None)
                    return redirect(url_for('register_step2'))
                
                person_id = generate_id()
                
                # Store session data in variables before clearing
                ...
                flash(get_flash_message(f'স্বাগতম {first_name}!', f'Welcome {first_name}!'), 'success')
                return redirect(url_for('dashboard'))
